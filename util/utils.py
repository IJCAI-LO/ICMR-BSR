...
def save_sweep_json(args, dataset_name: str, average_res: dict, out_dir="./sweep_logs"):
    """
    Save a single JSON record for param sweep.
    It tries to fetch AUROC/PRO and optionally Dice if present.
    """
    os.makedirs(out_dir, exist_ok=True)

    # helper: safely get metric
    ...
    # You may need to adjust these keys to match your calculate_metrics() output
    image_auroc = _get(average_res, "Sample_CLS", "AUROC", default=np.nan)
    pixel_auroc = _get(average_res, "Pixel", "AUROC", default=np.nan)
    pro        = _get(average_res, "Pixel", "PRO",   default=np.nan)

    # Dice key varies across repos; try common names
    dice = _get(average_res, "Pixel", "Dice", default=np.nan)
    if not np.isfinite(dice):
        dice = _get(average_res, "Pixel", "DICE", default=np.nan)
    if not np.isfinite(dice):
        dice = _get(average_res, "Pixel", "F1", default=np.nan)

    payload = {
        "dataset": dataset_name,
        "suffix": getattr(args, "suffix", ""),
        "lambda_attn_rcs": float(getattr(args, "lambda_attn_rcs", np.nan)),
        "sfr_sigma": float(getattr(args, "sfr_sigma", np.nan)),
        "sfr_thresh": float(getattr(args, "sfr_thresh", np.nan)),
        "eval_gauss_sigma": float(getattr(args, "eval_gauss_sigma", np.nan)),
        "metrics": {
            "Image_AUROC": float(image_auroc) if image_auroc is not None else float("nan"),
            "Pixel_AUROC": float(pixel_auroc) if pixel_auroc is not None else float("nan"),
            "PRO": float(pro) if pro is not None else float("nan"),
            "Dice": float(dice) if dice is not None else float("nan"),
        }
    }

    out_path = os.path.join(out_dir, f"{payload['suffix']}.json")
    with open(out_path, "w") as f:
        json.dump(payload, f, indent=2)

    print(f"[SWEEP] saved json -> {out_path}")
    ...
    with torch.no_grad():
        ...
        for data in test_dataloader:
            ...
            if torch.sum(labels) >= 1:
                ...
                _, anomaly_maps = clip_model.detect_forward(imgs, args, img_path=None)
...
def evaluation_pixel(clip_model, dataset_name, dataloader, args, device):
    pixel_gt_list = []
    pixel_score_list = []
    sample_gt_list = []
    sample_score_list = []
    ...
    # --- NEW: vis save dir ---
    save_vis = int(getattr(args, "save_vis", 0)) == 1
    ...
    with torch.no_grad():
        for items in tqdm(dataloader):
            imgs, labels, gt, category, img_path = items[:5]
            imgs = imgs.to(device)

            # -------------------------
            # forward (this will save maps via _maybe_save_stage inside model)
            # -------------------------
            predict_labels, predict_masks = clip_model.detect_forward(imgs, args, img_path=img_path)

            # -------------------------
            # NEW: save GT aligned to model naming
            # -------------------------
            # only save pixel-level gt for datasets that have pixel gt
            if save_vis and (dataset_name not in ['br35h', 'brainmri', 'headct']):
                # model wrote something like: "0001_hole__000.png"
                prefix = getattr(clip_model, "_last_vis_prefix", None)

                if prefix is not None:
                    # take batch0 to match _maybe_save_stage behavior
                    gt0 = gt[0]  # could be [1,H,W] or [H,W]
                    if gt0.ndim == 3:
                        gt0 = gt0[0]  # [H,W]

                    gt0 = gt0.to(device).float()
                    gt0 = (gt0 > 0.5).to(torch.uint8).cpu().numpy()  # 0/1

                    np.save(os.path.join(save_dir, f"{prefix}_gt.npy"), gt0)

            # -------------------------
            # normalize predict_masks type
            # -------------------------
            if isinstance(predict_masks, np.ndarray):
                predict_masks = torch.from_numpy(predict_masks).to(imgs.device)

            # resize to image resolution for metric computation (your original logic)
            predict_masks = F.interpolate(
                predict_masks,
                size=(imgs.size(-2), imgs.size(-1)),
                mode='bilinear',
                align_corners=False
            ).cpu().numpy()  # [B,1,H,W] -> numpy

            # --- key fix: adjust gaussian sigma when using refinement ---
            sigma = 4.0
            if getattr(args, "use_rcs", 0) == 1 or getattr(args, "use_sfr", 0) == 1:
                sigma = float(getattr(args, "eval_gauss_sigma", 2.0))  # 推荐 2.0 起步

            # apply gaussian per sample (keep exactly your behavior)
            predict_masks = np.stack([gaussian_filter(mask, sigma=sigma) for mask in predict_masks])

            # -------------------------
            # sample-level metrics buffers
            # -------------------------
            sample_gt_list.append(labels)
            sample_score_list.append(predict_labels.cpu().numpy())

            # -------------------------
            # pixel-level metrics buffers
            # -------------------------
            if dataset_name not in ['br35h', 'brainmri', 'headct']:
                gt_bin = gt.clone()
                gt_bin[gt_bin > 0.5] = 1
                gt_bin[gt_bin <= 0.5] = 0
                gt_bin = gt_bin.cpu().numpy().astype(int)  # [B,1,H,W] maybe

                # recompute image labels from pixel gt (your original logic)
                labels = np.max(gt_bin.reshape(gt_bin.shape[0], -1), axis=-1)

                pixel_gt_list.append(gt_bin)
...
def eval_all_class(clip_model: CLIP, dataset_name, test_dataset, args, logger, device):
    ...
    for category in test_dataset.categories:
        ...
        if args.fewshot > 0:
            ...
            print("fewshot paths:", len(fewshot_dataset.cur_img_paths), "fewshot=", args.fewshot)
    ...
    if args.vis == 0:
        ...
        # NEW: save sweep json for param sensitivity plots
        save_sweep_json(args, dataset_name, average_res, out_dir="./sweep_logs")
