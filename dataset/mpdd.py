...
class MPDDDataset(Dataset):
    def __init__(self, root, train=True, category=None, fewshot=0, transform=None, gt_target_transform=None):
        ...
        self.categories = ['bracket_black', 'bracket_brown', 'bracket_white', 'connector', 'metal_plate', 'tubes']

        self.train = train
        self.category = category
        self.fewshot = fewshot
        self.root = os.path.join(root, 'mpdd')
        ...
        self.dataset_name = "mpdd"

    def preprocess(self):
        self.img_paths = {'train': {category: [] for category in self.categories},
                          'test': {category: [] for category in self.categories}}
        self.gt_paths = {'train': {category: [] for category in self.categories},
                         'test': {category: [] for category in self.categories}}
        self.labels = {'train': {category: [] for category in self.categories},
                       'test': {category: [] for category in self.categories}}
